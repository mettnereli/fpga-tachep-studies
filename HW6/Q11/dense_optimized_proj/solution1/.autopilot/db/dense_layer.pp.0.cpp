...
__attribute__((sdx_kernel("dense_optimized", 0))) void dense_optimized(
    int x[16],
    int w[16][8],
    int y[8]
);
...
void dense_simple(
    ...
    VITIS_LOOP_8_1: for (int j = 0; j < 8; j++) {
        int sum = 0;

        VITIS_LOOP_11_2: for (int i = 0; i < 16; i++) {
            sum += x[i] * w[i][j];
        }
...
#line 14 "/scratch/emettner/fpga-tachep-studies/HW6/Q11/run_dense_layer.tcl"
#pragma HLSDIRECTIVE TOP name=dense_optimized
# 23 "dense_layer.cpp"
...
#pragma HLS ALLOCATION operation instances=mul limit=4
 ...
 VITIS_LOOP_35_1: for (int j = 0; j < 8; j++) {
        int sum = 0;

        VITIS_LOOP_38_2: for (int i = 0; i < 16; i++) {
