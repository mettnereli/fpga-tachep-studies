...
void add_int(int a[16], int b[16], int c[16]);

void add_long(long a[16], long b[16], long c[16]);

__attribute__((sdx_kernel("add_longlong", 0))) void add_longlong(long long a[16], long long b[16], long long c[16]);

void add_ap_int(ap_int<32> a[16], ap_int<32> b[16], ap_int<32> c[16]);

void add_ap_uint(ap_uint<32> a[16], ap_uint<32> b[16], ap_uint<32> c[16]);

void add_ap_fixed(ap_fixed<16,6> a[16], ap_fixed<16,6> b[16], ap_fixed<16,6> c[16]);
# 2 "q3_kernels.cpp" 2

void add_int(int a[16], int b[16], int c[16]) {
    VITIS_LOOP_4_1: for (int i = 0; i < 16; i++) {
        c[i] = a[i] + b[i];
    }
}
...
__attribute__((sdx_kernel("add_longlong", 0))) void add_longlong(long long a[16], long long b[16], long long c[16]) {
#line 14 "/scratch/emettner/fpga-tachep-studies/HW2/Q3/run_q3_kernels.tcl"
#pragma HLSDIRECTIVE TOP name=add_longlong
# 15 "q3_kernels.cpp"

    VITIS_LOOP_16_1: for (int i = 0; i < 16; i++) {
        c[i] = a[i] + b[i];
    }
}
...
void add_ap_uint(ap_uint<32> a[16], ap_uint<32> b[16], ap_uint<32> c[16]) {
    VITIS_LOOP_28_1: for (int i = 0; i < 16; i++) {
        c[i] = a[i] + b[i];
    }
}

void add_ap_fixed(ap_fixed<16,6> a[16], ap_fixed<16,6> b[16], ap_fixed<16,6> c[16]) {
    VITIS_LOOP_34_1: for (int i = 0; i < 16; i++) {
        c[i] = a[i] + b[i];
    }
}
